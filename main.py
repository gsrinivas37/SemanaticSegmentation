...
def load_vgg(sess, vgg_path):
    ...
    return l_input, l_keep, l3, l4, l7
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # 1X1 convolution
    input = tf.layers.conv2d(vgg_layer7_out, num_classes, 1, padding='same',
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for image, label in get_batches_fn(batch_size):
            train,loss = sess.run([train_op,cross_entropy_loss],
...
def run():
    ...
    with tf.Session() as sess:
        ...
        l_input, l_keep, l3, l4, l7 = load_vgg(sess, vgg_path)
        layer_output = layers(l3, l4, l7, num_classes)
        logits, train_op, cross_entropy_loss = optimize(layer_output, correct_label, learning_rate, num_classes)
        
        # Train NN using the train_nn function
        sess.run(tf.global_variables_initializer())
        train_nn(sess, 20, 16, get_batches_fn, train_op, cross_entropy_loss, l_input,
             correct_label, l_keep, learning_rate)

        # Save inference data using helper.save_inference_samples
        helper.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, l_keep, l_input)
